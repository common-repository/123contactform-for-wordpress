<?php
/*
Plugin Name: 123FormBuilder for WordPress
Plugin URI: http://www.123formbuilder.com/wordpress-contact-form-plugin.html
Description: Form Builder plugin from 123FormBuilder.com. Usage notes, tips and tricks, <a href="http://www.123formbuilder.com/wordpress-contact-form-plugin.html">here</a>.
Author: 123FormBuilder.com
Version: 1.5.6
...
function w123cf_widget_text_filter( $content ) {
    ...
    while ($ready==false)
    {
        ...
        if (strpos($tosearch, '[123-form-builder') !== false) {
            $i = strpos($tosearch, "[123-form-builder ");
        }

        if (strpos($tosearch, '[123-contact-form-eu') !== false) {
            $i = strpos($tosearch, "[123-contact-form-eu ");
            $domain = 'eu.' . $rootDomain;
        }

        if (strpos($tosearch, '[123-form-builder-eu') !== false) {
            $i = strpos($tosearch, "[123-form-builder-eu ");
            $domain = 'eu.' . $rootDomain;
        }
		...
		$pos_custom_controls=strpos($tosearch,"control",$i+19);
	        if(($pos_custom_controls > $i) &&($pos_custom_controls < $j))
                ...
                if (strpos($tosearch, '[123-contact-form-eu') !== false || strpos($tosearch, '[123-form-builder-eu') !== false)
	        ...
	        if (is_numeric($id))
				...
				if ( is_callable('curl_init') ) {					
					$curl = curl_init("https://" . $domain . "/embedded-link/" . $id . ".txt");
				...
				elseif ( is_callable('file_get_contents') ) {
					$linkcode=file_get_contents("https://".$domain."/embedded-link/".$id.".txt");
					}
...
function cfp_connect() {
        ...
        echo cfp_message("error: " . openssl_error_string(), 0);
        exit();
}
    exit();
}
...
function cfp_custom_main_js() {
    wp_enqueue_style( 'cfp-main-css', plugin_dir_url( __FILE__ ) . 'admin/css/cpf_main.css', array(), '', false );
    wp_enqueue_script( 'cfp-main-js', plugin_dir_url( __FILE__ ) . 'admin/js/cpf_main.js', array(), '', true );
}
add_action( 'admin_enqueue_scripts', 'cfp_custom_main_js' );
